...
client.on('ready', async () => {
    ...
    client.user.setPresence({ activities: [{ name: 'MTX | .العاب', type: 3 }], status: 'dnd' });
    const cmds = [
    ...
    try { await client.application.commands.set(cmds); } catch (e) { console.log('slash err: ' + e.message); }
});
...
client.on('guildMemberAdd', async member => {
    ...
    let owner;
    try { owner = await guild.fetchOwner(); } catch (e) { return; }
    if (!(await protOn(guild.id))) return;
    try {
    ...
    } catch (e) { console.log('anti bot err: ' + e.message); }
});
...
client.on('messageCreate', async msg => {
    ...
    if (!isAdmin) {
        ...
        if (userSpam.length >= 5) {
            ...
            } catch (e) { console.log('spam mute err: ' + e.message); }
            return;
        }
    }
    ...
    if (!isAdmin && !isOwner) {
        if (linkRegex.test(msg.content)) {
            try {
                ...
                setTimeout(() => m.delete().catch(() => {}), 10000);
                sendLog(msg.guild, embed(blue, '📝 auto mute', 'user: ' + msg.author + '\nreason: link\nmod: bot'));
            } catch (e) { console.log('link mute err: ' + e.message); }
            return;
        }
    }
    ...
    if (cmd === 'باند') {
        ...
        } catch (err) { msg.reply({ embeds: [embed(red, '❌ error', err.message)] }); }
    }
        ...
        if (count >= 5) {
            ...
            } catch (err) { msg.reply({ embeds: [embed(red, '❌ auto mute failed', err.message)] }); }
            return;
        }
        ...
        const e = new EmbedBuilder().setTitle('warnings for ' + target.user.tag).setDescription(text).setColor(yellow).setFooter({ text: warnings.length + '/5 warnings | 5 = 2 day mute' });
        msg.reply({ embeds: [e] });
    }
        ...
        msg.reply({ embeds: [new EmbedBuilder().setTitle('🎮 games').setDescription('choose a game').setColor(green)], components: [row] });
    }
});
...
client.on('interactionCreate', async i => {
    ...
    } else if (i.commandName === 'حالة') {
        const uptime = Date.now() - startTime;
        const hours = Math.floor(uptime / 3600000);
        const mins = Math.floor((uptime % 3600000) / 60000);
        const e = new EmbedBuilder().setTitle('🤖 MTX status').setDescription('online for ' + hours + 'h ' + mins + 'm').setColor(green).addFields({ name: 'protection', value: (await protOn(i.guildId)) ? 'on' : 'off', inline: true }, { name: 'guilds', value: String(client.guilds.cache.size), inline: true });
        i.reply({ embeds: [e] });
    }
});
